...
$con= mysqli_connect(HOST,USER,PW,DB) or die("Unable to Connect");
		...
		$sql = "INSERT INTO accountInfoGirls (userName,userID,userEmail,userCell,userPW,userPic,userHeight,userAge,userJob) VALUES (?,?,?,?,?,?,?,?,?)";
		...
		$check = mysqli_stmt_affected_rows($stmt);
                ...
                $userGender="girls";
                 $userHolding="notRatified";
		$sql2= "INSERT INTO logSet (userID,userEmail,userPW,Gender,userHolding) VALUES (?,?,?,?,?)";
		$stmt2 = mysqli_prepare($con,$sql2);
		mysqli_stmt_bind_param($stmt2,"sssss", $userID,$userEmail,$pwEncoded,$userGender,$userHolding);
		mysqli_stmt_execute($stmt2);
		$check2 = mysqli_stmt_affected_rows($stmt2);
		
		
				$sql3= "INSERT INTO detailedInfoGirls1 (userID,userCell,userHeight,userJob,userHolding,userEmail,userAge,userPicSmall,userPicMain,userRegion,userRegion2,userUniversity,userKey,userKeyCalc) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		$stmt3 = mysqli_prepare($con,$sql3);
		mysqli_stmt_bind_param($stmt3,"ssssssssssssss", $userID,$userCell,$userHeight,$userJob,$userHolding,$userEmail,$userAge,$userPicSmall,$userPic,$userRegion,$userRegion2,$userUniversity,$userKeyFirst,$keyJson);
		mysqli_stmt_execute($stmt3);
		$check3 = mysqli_stmt_affected_rows($stmt3);

  		$sql4= "INSERT INTO preferenceForGirls (userEmail,cardAge,cardRegion) VALUES (?,?,?)";
