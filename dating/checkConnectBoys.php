<?php
	...
	$con= mysqli_connect(HOST,USER,PW,DB) or die("Unable to Connect");
		...
		$sql0 = "SELECT Liking,LikedBy,Connected from detailedInfoBoys1 where userEmail = '$userEmail'";
		...
		$Connected = $row0['Connected'];
			...
			for($i=0; $i<$ListCount2; $i++){
				if( 4 >$InputTime - $ArrayList2[$i]['InputTime']){
			        ...
			        $sql2 = "SELECT userID,userCell  from detailedInfoGirls1 where userEmail = '$makeEmail2'";
			...
			for($i=0; $i<$ListCount3; $i++){
				...
				if( 7 >$InputTime - $ArrayList3[$i]['InputTime']){
			        ...
			        $sql3 = "SELECT userID,userCell  from detailedInfoGirls1 where userEmail = '$makeEmail3'";
			...
			$messageCheck ="none";
			$sqlTimer ="UPDATE detailedInfoBoys1 SET messageListener=? WHERE userEmail=?";
